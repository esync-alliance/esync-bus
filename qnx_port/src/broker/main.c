...
static int fd;
static poll_data_t main_pit;
static poll_info_t poll_info = { .polls = 0, .polls_len = 0};
...
int set_poll(xl4bus_connection_t * conn, int fd, int flg) {
    ...
    if ((!ci) || ((ci) && (ci->poll_modes != need_flg))) {
        ...
        if (need_flg) {
            ...
            if (!found) {
				void * v = realloc(poll_info.polls, (poll_info.polls_len + 1) * sizeof(pf_poll_t));
				if (!v) {
            ...
            if ((ci) && (ci->poll_modes)) {
                for (int i=0; i<poll_info.polls_len; i++) {
                    if (poll_info.polls[i].fd == conn->fd) {
